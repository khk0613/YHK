...
$tel3 = '';
if (isset($_POST['tel3']) && $_POST['tel3']) { // 해당값이 넘어왔는가, 해당 값이 실제 존재하는가. 앞에 값은 공백으로 넘어와도 참이 되고 뒤에 것은공백으로 넘어오믄 거짓이 됩니다.즉 공북으로 넘어오면 처리하지 말라는 말과 같습니다. 이해했습니
    $tel3 = $_POST['tel3'];
}
/* --- 2015. 09. 06 시공사진 데이터 필드 추가--- */
$baljuchu = '';
if (isset($_POST['baljuchu']) && $_POST['baljuchu']) {
    $baljuchu = $_POST['baljuchu'];
}

$witch = '';
if (isset($_POST['witch']) && $_POST['witch']) {
    $witch = $_POST['witch'];
}

$nyundo = '';
if (isset($_POST['nyundo']) && $_POST['nyundo']) { // 해당값이 넘어왔는가, 해당 값이 실제 존재하는가. 앞에 값은 공백으로 넘어와도 참이 되고 뒤에 것은공백으로 넘어오믄 거짓이 됩니다.즉 공북으로 넘어오면 처리하지 말라는 말과 같습니다. 이해했습니
    $nyundo = $_POST['nyundo'];
}

$yeongyeok = '';
if (isset($_POST['yeongyeok']) && $_POST['yeongyeok']) {
    $yeongyeok = $_POST['yeongyeok'];
}

$sulmyung = '';
if (isset($_POST['sulmyung']) && $_POST['sulmyung']) {
    $sulmyung = $_POST['sulmyung'];
}
/* --- 2015. 09. 06 --- */
...
if ($w == '' || $w == 'r') {
    ...
    if ( $write_table == 'g5_write_qna' ) { // 이부분이 자못되넉 같은데 qna 가 아니고 다른 이름인듯 
        $sql .= "
            , tel1 = '$tel1' 
            , tel2 = '$tel2'
            , tel3 = '$tel3'
    ...
    if ( $write_table == 'g5_write_roof' || $write_table == 'g5_write_floor' || $write_table == 'g5_write_cyro' ) { // 이부분이 자못되넉 같은데 qna 가 아니고 다른 이름인듯 
        $sql .= "
            , baljuchu = '$baljuchu' 
            , witch = '$witch'
            , nyundo = '$nyundo'
